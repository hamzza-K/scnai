...
WHERE [System.WorkItemType] = 'User Story'
  AND [System.Tags] NOT CONTAINS 'Internal Tools'
  AND [System.Tags] CONTAINS 'NOT IN SCN'
  AND [System.IterationPath] UNDER '{iteration_path}'
ORDER BY [Custom.WeightedPriority] ASC
"""
...
def fetch_scn_user_stories(wit_client: Any, iteration_path: str) -> list[dict[str, Any]]:
    wiql_query = Wiql(query=SCN_WIQL_TEMPLATE.format(iteration_path=iteration_path))
    story_refs = wit_client.query_by_wiql(wiql_query).work_items
    stories: list[dict[str, Any]] = []
    total = len(story_refs)
    for index, item in enumerate(story_refs, start=1):
        ...
        if index % 25 == 0 or index == total:
            logger.info("Fetched SCN work items %s/%s", index, total)
...
def fetch_scn_bugs(wit_client: Any, iteration_path: str) -> list[dict[str, Any]]:
    wiql_query = Wiql(query=BUGS_SCN_WIQL_TEMPLATE.format(iteration_path=iteration_path))
    bug_refs = wit_client.query_by_wiql(wiql_query).work_items
    bugs: list[dict[str, Any]] = []
    total = len(bug_refs)
    ...
    for index, item in enumerate(bug_refs, start=1):
        ...
        if index % 25 == 0 or index == total:
            logger.info("Fetched SCN bugs %s/%s", index, total)
